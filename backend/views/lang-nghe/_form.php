...
<div class="lang-nghe-form">

    <?php $form = ActiveForm::begin(); ?>

    <?= $form->field($model, 'TenLangNghe')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'TomTat')->textarea(['rows' => 4]) ?>

    <?= $form->field($model, 'MoTaChiTiet')->textarea(['rows' => 6,'id'=>'content']) ?>


    <div class="row">
        <div class="col-md-4 col-4">
            <?= $form->field($model, 'ToaDo')->textInput(['maxlength' => true]) ?>
        </div>
        <div class="col-md-4 col-4">
            <?=$form->field($model, 'QuanHuyen_id')->widget(\kartik\select2\Select2::classname(), [
                'data' => \yii\helpers\ArrayHelper::map(
                    \common\models\QuanHuyen::find()->OrderBy(['TenQuanHuyen'=>3])->all(),
                    'id',
                    'TenQuanHuyen'
                ),
                'language' => 'de',
